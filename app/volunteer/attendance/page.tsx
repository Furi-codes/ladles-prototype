...
export default function AttendancePage() {
  ...
  const record = useCallback(async () => { if (!checkpoint) return; setIsSaving(true); const { data, error } = await recordEventAttendance(checkpoint); setIsSaving(false); if (error) { setMessage(error.message); return; } await refreshData(); setMessage(data?.clocked_out_at ? `Clock-out recorded. Your recorded time is ${formatWorkedTime(data.worked_minutes)}.` : "Clock-in recorded successfully."); }, [checkpoint, refreshData]);
