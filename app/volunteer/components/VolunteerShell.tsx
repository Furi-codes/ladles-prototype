...
export default function VolunteerShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className={styles.shell}><VolunteerHeader profile={profile} avatarUrl={avatarUrl} onSignOut={() => setShowSignOutConfirm(true)} /><main className={styles.content}>{loadError && <div className={styles.error} role="alert">{loadError}</div>}{children}</main><ToastContainer toasts={toasts} />{showSignOutConfirm && <ConfirmDialog title="Sign out?" message="Are you sure you want to sign out of the volunteer portal?" confirmLabel="Sign out" onCancel={() => setShowSignOutConfirm(false)} onConfirm={handleSignOut} />}</div>;
}
