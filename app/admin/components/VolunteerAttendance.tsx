...
export default function VolunteerAttendance({ events, bookings }: { events: Event[]; bookings: Booking[] }) {
  ...
  return <section className={styles.card} aria-labelledby="attendance-title">
    <div className={styles.cardHeader}>
      <div><h2 id="attendance-title" className={styles.cardTitle}>Attendance</h2><p className={styles.cardHint}>Select an event to view and update its roster.</p></div>
    ...
    {selectedEventId === "" ? <div className={styles.empty}>Select an event to view its volunteer roster.</div> : <div className={styles.tableWrap}>
      <table className={styles.table}>
        <thead><tr>{["Volunteer", "Time range", "Status", "Action"].map((column) => <th key={column}>{column}</th>)}</tr></thead>
        <tbody>
          {currentRoster.map((booking) => {
            const present = booking.status === "Present";
            const completed = booking.status === "Completed";
            const statusClass = present ? styles.statusPresent : completed ? styles.statusCompleted : booking.status === "No show" ? styles.statusCancelled : styles.statusConfirmed;
            ...
            return <tr key={booking.id}>
              ...
              <td>{completed ? <span className={styles.cardHint}>Recorded by QR</span> : <button type="button" className={present ? styles.dangerButton : styles.secondaryButton} onClick={() => handleToggleAttendance(booking.id, booking.status)}>{present ? "Undo" : "Mark present"}</button>}</td>
            </tr>;
          })}
          {currentRoster.length === 0 && <tr><td colSpan={4}><div className={styles.empty}>No volunteers have booked this event.</div></td></tr>}
        </tbody>
      </table>
    </div>}
  </section>;
